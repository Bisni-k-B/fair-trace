...
import { WinningStrategy } from './engine/crcn';
...
const App: React.FC = () => {
  ...
  const createRequest = async (urgency?: Urgency, customData?: { location: string, issue: string, source?: 'web' | 'whatsapp' | 'telegram' | 'instagram' }) => {
  ...
  const _simulateCrash = (idx: number) => {
    if (engine.slots[idx].node) {
  ...
  const _getEffectiveScore = (node: ProcessNode) => {
            ...
            <div className="situation-room-canvas">
               <svg className="neural-graph-svg">
                  ...
                  {clusters.map((cluster, cIdx) => 
                    cluster.engine.slots.map((slot, _sIdx) => {
                          ...
                          {node ? (
                            <motion.div key={node.id} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className={`node ${node.urgency === 3 ? 'critical' : node.urgency === 2 ? 'high' : 'medium'}`} style={{ width: '100%', margin: 0 }}>
                              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                ...
                                {node.source === 'instagram' && <Activity size={12} color="#E1306C" />}
                             </div>
